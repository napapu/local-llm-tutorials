...
2. Follow the [documentation](https://lmstudio.ai/docs/app/basics)

## (Advanced) Building llama.cpp and downloading your first brain
1. Install [brew](https://brew.sh/)
> For installing build dependencies for llama.cpp

2. Create a directory for storing LLMs
    - `mkdir ~/gguf`
> See [GGUF](https://huggingface.co/docs/transformers/en/gguf) for more information. 

3. Download your first LLM from huggingface.
...
4. Install build dependencies in brew
    - `brew install cmake ninja`
> This domain moves fast, we build from source

5. Clone [llama.cpp](https://github.com/ggml-org/llama.cpp) to a known location i.e. `~/tools`
    - `git clone git@github.com:ggml-org/llama.cpp.git ~/tools/llama.cpp`
> The open-source tool for serving your local brain

6. Build it. Go to [the docs](https://github.com/ggml-org/llama.cpp/blob/master/docs/build.md) for more.
    - a) `cd ~/tools/llama.cpp`
    - b) `cmake -B build -G Ninja`
    - c) `cmake --build build -j`
> Step b) should have ended with `Build files have been written to: /Users/upapan/tools/llama.cpp/build` and step c) with `Linking CXX executable bin/llama-server`. `llama-server` is the thing you want most but there's a lot of other tools in there.

7. (Optional) Add `~/tools/llama.cpp/build/bin` to your PATH
    - `echo 'export PATH="$PATH:$HOME/tools/llama.cpp/build/bin"' >> ~/.zshrc`
    - `source ~/.zshrc`
> Makes it easier to execute from anywhere in `zsh`

8. Execute `llama-server`. You should see `main: router server is listening on http://127.0.0.1:8080`
> Validates that it built and can run successfully.

9. Serve it with the LLM.
```
llama-server \
