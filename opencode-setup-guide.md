...
It will be pre-selected to the free model available. Change it to Qwen3.5-9B that we've just set up.
...
## OpenCode 
This assumes that your custom provider for LlamaCPP - Qwen3.5-9B has been set up.

1. Install [OpenCode Terminal](https://opencode.ai/download).
2. `cd` to your playground repository.
3. Run `opencode`.
4. `Ctrl + p` for commands.
5. `Switch model` > `Qwen3.5-9B LlamaCPP`. Hit return.
